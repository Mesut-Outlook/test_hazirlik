...
| A4a | `extract.py` YAPISAL düzeltme (X1 bulguları: çok maddeli soruların satır/alt-öğe yapısı, blok üst üste binmesi, kök çizgisi taşması, cevap anahtarı konumu, sütun dağılımı) + yeniden üretim → `build_linux/v3_taslak2.pdf` (v3_taslak.pdf'in ÜZERİNE YAZMA — QA ajanları hâlâ okuyor) + SISTEM.md §6 doğrulama | Claude-Sonnet #5 → #6 | 🔄 devam ediyor — #5 oturum limitinde kesildi (extract.py düzeltmeleri + v3_taslak2.pdf 185 sf ÜRETİLDİ, doğrulama yarım). Fable ara doğrulaması (2026-07-05 11:35): yapısal düzeltmeler ÇALIŞIYOR (sf.60 = v2 sf.50 gözle teyit: satır yapısı, kesirler, kökler, şık boşlukları düzgün; soru metni tam 120=120), AMA: (1) REGRESYON — .kur-tag/.tag-kurgusu çıkarımda kayboluyor ("GÜNLÜK HAYAT" 7→0, "ÖSYM SORULARINA" 10→0, "PİST ALANI" 8→0, "ISINMA" 15→4, "Kurgusu" 20→4; eski taslakta tamdı); (2) kök içi kesir düzleşmesi (√(1−11/36) → "√(1−11) 36"); (3) cevap anahtarında kesir kaybı (5/6→"5 6"); (4) alt-öğe etiket sırası yer yer bozuk; (5) sayfa 136→185 şişmesi incelenmeli. #6 bu listeyle devam ediyor |
